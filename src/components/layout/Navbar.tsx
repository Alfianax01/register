...
import { Shield, Search, LogIn, Menu, X, Home, UserPlus, Ticket, ChevronRight } from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 w-full h-[72px] bg-white/95 backdrop-blur-md border-b border-slate-200/80 transition-all">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2.5">
            ...
            <Link
              href="/admin/login"
              className="inline-flex items-center gap-1.5 text-[13px] font-medium text-[#0F172A] bg-slate-100 hover:bg-slate-200/80 px-3.5 py-2 rounded-lg border border-slate-200/80 shadow-xs transition-colors"
            >
          ...
          <aside className="fixed inset-y-0 right-0 w-80 max-w-[85vw] bg-white shadow-2xl flex flex-col z-10 animate-in slide-in-from-right duration-200">
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/50 space-y-2">
              <span className="text-[11px] text-slate-400 font-medium block">
                Akses Khusus Panitia & Petugas:
              </span>
              <Link
                href="/admin/login"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full flex items-center justify-center gap-2 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-50 py-2.5 rounded-lg border border-slate-200 shadow-xs transition-colors"
              >
                <LogIn className="w-3.5 h-3.5 text-slate-500" />
                <span>Login Portal Petugas Gate</span>
              </Link>
            </div>
          </aside>
        </div>
